// src/lib/ai-tools/instagramProfileTool.js
export async function executeGetInstagramProfile(profileUrl, apiToken) {
  let toolOutput = 'Fetching Instagram profile for ' + profileUrl + '\\n';
  const datasetId = 'gd_l1vikfch901nx3by4'; // Dataset ID for Instagram Profiles

  if (!apiToken) {
    console.error("BrightData API token not configured for Instagram Profile tool.");
    toolOutput += "Error: BrightData API credentials not configured.";
    return toolOutput;
  }
  // Add URL validation if needed (e.g., must contain 'instagram.com/')
  if (!profileUrl || typeof profileUrl !== 'string' || !profileUrl.includes('instagram.com/')) {
    console.error("Invalid or missing Instagram profile URL:", profileUrl);
    toolOutput += "Error: A valid Instagram profile URL is required.";
    return toolOutput;
  }

  try {
    console.log('Triggering BrightData dataset for Instagram profile: ' + profileUrl);
    ...
    while (attempts < maxAttempts) {
      ...
      attempts++;
      console.log('Polling snapshot ' + snapshotId + ' for Instagram profile, attempt ' + attempts + '/' + maxAttempts);
      ...
      console.log('Instagram profile snapshot data received.');
    ...
    console.error("Error with Instagram Profile tool:", apiError);
    toolOutput = (toolOutput.startsWith("Fetching") ? toolOutput : "") + 'Exception during Instagram Profile tool: ' + apiError.message;
  }
  return toolOutput;
}