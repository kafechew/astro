// src/lib/ai-tools/instagramReelsTool.js
export async function executeGetInstagramReels(instagramUrl, apiToken) {
  let toolOutput = 'Fetching Instagram reels for ' + instagramUrl + '\\n';
  const datasetId = 'gd_lyclm20il4r5helnj'; // Dataset ID for Instagram Reels

  if (!apiToken) {
    console.error("BrightData API token not configured for Instagram Reels tool.");
  ...
  if (!instagramUrl || typeof instagramUrl !== 'string' || !instagramUrl.includes('instagram.com/')) {
    console.error("Invalid or missing Instagram URL for Reels tool:", instagramUrl);
    toolOutput += "Error: A valid Instagram URL (profile or reel) is required.";
    return toolOutput;
  }

  try {
    console.log('Triggering BrightData dataset for Instagram reels: ' + instagramUrl);
    ...
    while (attempts < maxAttempts) {
      ...
      attempts++;
      console.log('Polling snapshot ' + snapshotId + ' for Instagram reels, attempt ' + attempts + '/' + maxAttempts);
      ...
      console.log('Instagram reels snapshot data received.');
    ...
    console.error("Error with Instagram Reels tool:", apiError);
    toolOutput = (toolOutput.startsWith("Fetching") ? toolOutput : "") + 'Exception during Instagram Reels tool: ' + apiError.message;
  }
  return toolOutput;
}