// src/lib/ai-tools/xPostsTool.js
export async function executeGetXPosts(postUrl, apiToken) {
  let toolOutput = 'Fetching X (Twitter) post data for: ' + postUrl + '\\n';
  const datasetId = 'gd_lwxkxvnf1cynvib9co'; // Dataset ID for X Posts

  if (!apiToken) {
    console.error("BrightData API token not configured for X Posts tool.");
    toolOutput += "Error: BrightData API credentials not configured.";
    return toolOutput;
  }
  // Add URL validation if needed (e.g., must contain 'x.com/' or 'twitter.com/')
  if (!postUrl || typeof postUrl !== 'string' || !(postUrl.includes('x.com/') || postUrl.includes('twitter.com/'))) {
    console.error("Invalid or missing X (Twitter) post URL:", postUrl);
    toolOutput += "Error: A valid X (Twitter) post URL is required.";
    return toolOutput;
  }

  try {
    console.log('Triggering BrightData dataset for X post: ' + postUrl);
    ...
    while (attempts < maxAttempts) {
      ...
      attempts++;
      console.log('Polling snapshot ' + snapshotId + ' for X post, attempt ' + attempts + '/' + maxAttempts);
      ...
      console.log('X post snapshot data received.');
    ...
    console.error("Error with X Posts tool:", apiError);
    toolOutput = (toolOutput.startsWith("Fetching") ? toolOutput : "") + 'Exception during X Posts tool: ' + apiError.message;
  }
  return toolOutput;
}