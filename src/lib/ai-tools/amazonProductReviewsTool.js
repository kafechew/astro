// src/lib/ai-tools/amazonProductReviewsTool.js
export async function executeGetAmazonProductReviews(productUrl, apiToken) {
  let toolOutput = 'Fetching Amazon product reviews for ' + productUrl + '\\n';
  const datasetId = 'gd_le8e811kzy4ggddlq'; // Dataset ID for Amazon Product Reviews

  if (!apiToken) {
    console.error("BrightData API token not configured for Amazon Product Reviews tool.");
    toolOutput += "Error: BrightData API credentials not configured.";
    return toolOutput;
  }
  if (!productUrl || typeof productUrl !== 'string' || !productUrl.includes('/dp/')) {
    console.error("Invalid or missing Amazon product URL (must contain /dp/) for Amazon Product Reviews tool:", productUrl);
  ...
  try {
    ...
    while (attempts < maxAttempts) {
      ...
      console.log('Amazon product reviews snapshot data received.'); // Removed verbose data log
